...
<body>
	...
	<!--START OF SIDEBAR -->
	<form id="sidebar">
		<a href="addvetandgrom.php" class="brand">
				<div class="logo">
		...
		<!--END OF NAVBAR -->
			...
			<div class="container">
				...
				<form action="#" method="POST" enctype="multipart/form-data">
					...
					<?php 
						$catid = $_GET['cat_ID'];
						$sqlict=mysqli_query($conn, "SELECT * FROM categories WHERE cat_ID = '$catid'");
						while($value=mysqli_fetch_array($sqlict)){
...
<!--Type of Services start-->
						...
						<input type="hidden" name="update_cat_id" placeholder="Business Name" class="col-md-6 form-control bg-white text-dark my-3 text-start" value="<?php echo $value['cat_ID']; ?>">
							<div class="services-pic">
								<img id="photo2" <?php echo '<img src="vetphoto/'.$value['cat_img'].'"'; ?>>
								<input type="file" name="update_img" id="file2">
							...
							<div class="input-field">
								...
								<input type="text" name="update_name" placeholder="Enter services name" required value="<?php echo $value ['cat_name']; ?>">
							</div>
							<div class="input-field">
								<label>Category  Description</label>
								<textarea type="text" name="update_description" placeholder="Enter services description" required><?php echo $value ['cat_description']; ?></textarea>
							</div>
						</div>	
						<div class="row">
							<input type="submit" name="update_categories" class="app-button" id="btnSearch" value="Update">
							<a href="admincategoriestable.php"><input type="button" class="app-button" value="Cancel"></a>
						  </div>
                       </div>
					</div>
					<?php } ?>
